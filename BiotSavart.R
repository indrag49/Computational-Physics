...
B <- function(X, Y, Z, a, length, I, n) {
  ...
  return(data.frame(B1, B2, L))
  #return(B2)
}

Z <- seq(0, 1, 0.001)
b <- B(0, 0, Z, 0.5, 1, 10^7/(4*pi), 1)$B2
plot(Z, b, type="l", col="red", main=paste("Bz Vs. Z, with X=0 and Y=",0), ylab="Bz")
for(i in seq(0.01, 1, 0.01)) {
  b <- B(0, i, Z, 0.5, 1, 10^7/(4*pi), 1)$B2
  plot(Z, b, type="l", col="red", main=paste("Bz Vs. Z, with X=0 and Y=",i), ylab="Bz")
  Sys.sleep(0.09)
}